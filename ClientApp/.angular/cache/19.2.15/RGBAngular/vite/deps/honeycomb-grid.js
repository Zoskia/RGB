...
function nt(r) {
  const {
    ...
    origin: n,
    offset: s
  } = __spreadValues(__spreadValues({}, T), r);
...
function qt(r, t, {
  ...
  const {
    ...
    col: i,
    row: c
  } = K(t, e, n), u = s < i ? "A" : "B", h = o < c ? "A" : "B", f = u + h, {
    swapWidthHeight: y,
    direction: m
  } = yt[f], O = Math.abs(s - i) + 1, g = Math.abs(o - c) + 1;
  return {
    width: y ? g : O,
    height: y ? O : g,
    start: r,
...
var q = class _q {
  ...
  static fromJSON({
    ...
    if (n) {
      const o = e.map(n), i = o.length > 0 ? o[0].constructor : n({
        q: 0,
        r: 0
      }, 0, [{
        q: 0,
    ...
    const s = nt(t);
    return new _q(s, e.map((o) => new s(o)));
  }
  get size() {
    return this.#r.size;
  }
  get pixelWidth() {
    ...
    const {
      isPointy: t,
      width: e
    } = this.hexPrototype, n = this.toArray(), {
      0: s,
      length: o,
      [o - 1]: i
